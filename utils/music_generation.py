...
def cut_prelude(song, cut_time_ratio):
    ...
    for track in song.tracks:
        ...
        cut_time_step = round(track_np.shape[0]*cut_time_ratio)
...
def music_generation_rnn(model, scaler_dict, track_order, prelude_tracks_dict, predict_time_step, device):
    ...
    for track in track_order:
        if scaler_dict!=None:
            ...
            transformed_track = prelude_tracks_dict[track]
...
def binarize_generated_music_dict(generated_tracks_dict, cutoff_p_dict, sigmoid, vol=85):
    """
    ...
    for track_name,track in generated_tracks_dict.items():
        if sigmoid:
            track = track.sigmoid()
        print(track_name, "mean: ", track.mean())
        new_tensor = torch.zeros(track.shape)
        new_tensor[track>cutoff_p_dict[track_name]] = vol
...
def check_vae_generation(model, dataset, device):
    """
    ...
    for song_seq, _ in dataset:
        ...
        transformed_track = model(song_seq).squeeze(1)
        transformed_tracks.append(transformed_track)
    original_tracks_tensor = torch.cat(original_tracks, dim=2).squeeze(0)
    print(original_tracks_tensor.shape)
    transformed_tracks_tensor = torch.cat(transformed_tracks, dim=2).squeeze(0)
    print(transformed_tracks_tensor.shape)
